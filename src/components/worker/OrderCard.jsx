import { MapPin, CreditCard, AlertTriangle, ChefHat, PackageCheck, CheckCheck } from 'lucide-react'
      ...
      <div className="bg-black/25 border border-bk-border rounded-xl p-3 mb-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          {order.items.map((item, i) => (
            <span
              key={i}
              className="inline-flex items-center gap-1 bg-bk-card2 border border-bk-border2
                         text-[11px] font-medium text-bk-muted rounded-lg px-2 py-1"
            >
              <span className="text-bk-primary text-[9px] font-bold">1×</span>
              {item.name}
            </span>
          ))}
        </div>
